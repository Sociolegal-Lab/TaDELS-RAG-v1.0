...
排行榜以 `Total` 呈現整體生成品質。`Total` 是所有題型的加權平均分數：短答題與長答題使用 Entity Matching Score，不可回答題使用 Exact Match。換言之，`Total` 同時反映系統回答可回答問題時抽取法律實體的正確性，以及面對不可回答問題時是否能正確拒答。

| 指標 | 使用欄位 | 適用題型 | 分數意義 |
| --- | --- | --- | --- |
| Total | `entities`、`answer` | 短答題、長答題、不可回答題 | 排行榜使用的整體生成品質分數；短答題與長答題納入 Entity Matching Score，不可回答題納入 Exact Match，再依題型題數加權平均 |
