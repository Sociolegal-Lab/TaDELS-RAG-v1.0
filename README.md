...
| validation |    101 |       34 |       34 |           33 | 供開發階段檢查系統表現使用         |
| test       |    101 |       34 |       34 |           33 | 供線上提交與結果比較使用           |
...
生成品質的設計借鏡問答評估中的 nugget-based evaluation：好的答案不只是一段語意相近的文字，而應涵蓋可逐項檢查的關鍵資訊。TREC Question Answering Track 以 nugget 評估答案是否包含必要資訊；近年的 TREC RAG Track 與 AutoNuggetizer 亦將此思路用於 RAG 系統評估。本研究在法律文書脈絡中將 nugget 具體化為法律實體欄位，逐項檢查案號、當事人、法條、裁判結果、行為事實與裁判理由等資訊是否正確，並另以幻覺相關指標標示無原文依據或與原文矛盾的內容。

線上系統的 leaderboard 以 `Total` 呈現整體生成品質。`Total` 是所有題型的加權平均分數：短答題與長答題使用 Entity Matching Score，不可回答題使用 Exact Match。換言之，`Total` 同時反映系統回答可回答問題時抽取法律實體的正確性，以及面對不可回答問題時是否能正確拒答。

| 指標 | 使用欄位 | 適用題型 | 分數意義 |
| --- | --- | --- | --- |
| Total | `entities`、`answer` | 短答題、長答題、不可回答題 | leaderboard 使用的整體生成品質分數；短答題與長答題納入 Entity Matching Score，不可回答題納入 Exact Match，再依題型題數加權平均 |
